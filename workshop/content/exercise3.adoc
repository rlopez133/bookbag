...
[{format_cmd_copy}]
----
ansible-rulebook -i inventory --rulebook exercise3-rulebook.yml --verbose --controller-url {aap_controller_web_url} --controller-token {aap_controller_token}
----
+
----
...
== Triggering the Rulebook

In this section, create the namespace, `eda` which will trigger the Ansible
Rulebook and the Ansible Job to set the proper resource quotas for the `eda`
namespace.

From the bottom terminal window, 
