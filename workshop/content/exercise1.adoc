...
:format_cmd_exec2: source,options="nowrap",,role="copy",subs="attributes"
...
position by pression the `i` key to enter insert mode.
====
+
...
In this section, we'll deploy an app called Rocket.Chat, an open-source
...
specific event, such as the creation of a Persistent Volume Claim, the
Rocket.Chat deployment will initiate this event, causing our Ansible Rulebook to
launch the Ansible Job Template 'EDA-OpenShift-Exercise1'.
...
process of providing a valid Lets Encrypt certificate. 
